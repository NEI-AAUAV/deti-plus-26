...
/**
 * Returns the complete public configuration for the event.
 *
 * Missing/invalid settings fall back to safe defaults.
 */
...
/**
 * Calculates the current public registration state.
 *
 * IMPORTANT:
 * In phase 1 this is informational only.
 * handleRegister_() does not enforce this state yet.
 *
 * Enforcement is added in phase 2, inside the existing ScriptLock.
 */
 ...
 *
 * Only exposes aggregate/non-sensitive event information.
 */
 ...
 *
 * This is intentionally compatible with the current schema:
 * - "registered"
 * - "cv_delivered"
 *
 * It is also already prepared for the future "waitlisted" state.
 */
function getRegistrationCounts_() {
  ...
  rows.forEach(function (entry) {
    const state = String(entry.record.state || '')
      .trim()
      .toLowerCase();
    ...
    // Current registrations use "registered" or "cv_delivered".
    // Rows without a state are also counted for backwards compatibility
    // with any legacy data already present in the sheet.
    registered++;
  });
